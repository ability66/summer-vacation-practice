...
public class TfIdfAlgorithm {
        ...
        try {
            ...
            if (!file.isDirectory()) {
                ...
                for (int i = 0; i < filelist.length; i++) {
                    if (filelist[i].contains(".~")) continue;
    ...
    private static String readFile(InputStream file) throws FileNotFoundException, IOException {
     ...
     */
    public static Map<String, Integer> segStr(String content, List<String> w) {
        if (content.contains(".docx")) {
            ...
            try {
                docx = new XWPFWordExtractor(new XWPFDocument(fileInputStream));
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        ...
        try {
            while ((lexeme = iks.next()) != null) {
                if (w.contains(lexeme.getLexemeText())) continue;
                if (words.containsKey(lexeme.getLexemeText())) {
                    words.put(lexeme.getLexemeText(), words.get(lexeme.getLexemeText()) + 1);
                } else {
                    words.put(lexeme.getLexemeText(), 1);
    ...
    public static Map<String, Integer> segStrTxt(String content, List<String> w) {
        // 分词
        Reader input = new StringReader(content);
        // 智能分词关闭（对分词的精度影响很大）
        IKSegmenter iks = new IKSegmenter(input, true);
     ...
     */
    public static Map<String, Map<String, Integer>> wordSegCount(String dir, List<String> w) {
        try {
            fileList = readDirs(dir);
            for (String filePath : fileList) {
                String content = readFile(filePath);
                Map<String, Integer> segs = segStr(content, w);
                allSegsMap.put(filePath, segs);
            }
        } catch (FileNotFoundException ffe) {
            ffe.printStackTrace();
    ...
    public static Map<String, Integer> wordSegCounts(String fileName, InputStream fileInputStream) {
        ...
        try {
            ...
            if (fileName.contains(".docx")) {
                ...
                content = readFile(fileInputStream);
            }
            Map<String, Integer> segs = segStr(content, new ArrayList<>());
            return segs;
        } catch (FileNotFoundException ffe) {
            ffe.printStackTrace();
        } catch (IOException io) {
    ...
    public static List<Map<String, Object>> wordSegCounts(String dir, List<String> w) {
        List<Map<String, Object>> list = new ArrayList<>();
        try {
            fileList = readDirs(dir);
            for (String filePath : fileList) {
                try {
                    Map<String, Object> allSegsMap = new HashMap<>();
                    File file = new File(filePath);
                    String content = "";
                    ...
                    Map<String, Integer> segs = segStrTxt(content, w);
                    allSegsMap.put("fileName", file.getName());
                    allSegsMap.put("result", segs);
                    list.add(allSegsMap);
                } catch (Exception e) {
                    continue;
                }
...
//      	}
        ...
        System.out.println(segStr("授权许可证是一串较长的包含字母数字和符号的字符串。获取后通过以下方法对SDK进行授权", new ArrayList<>()));
    }
}
